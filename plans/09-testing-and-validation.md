...
GC-root writes are deliberately absent** (§4.1.2): Nix 2.34.8's daemon protocol does not
implement `repairPath` even for root, and GC-root publication/removal is the
...
> §4.1.2): `repair` (destructive re-fetch/rebuild — unavailable through Nix 2.34.8's daemon
> protocol even for root, so the helper pins `--store local`) and any GC-root write
...
- **`verify` is strictly read-only** — it never mutates the store; the **separate destructive
  ...
  trigger a write (AC-S3, T-CACHE-3, AC-S21). Nix 2.34.8's daemon reports that `repairPath`
  is unsupported for both allowed-user and root clients — asserted as a negative parity test
  alongside the helper's fixed-local-store positive path (§4.3, AC-S21/AC-S28).
...
authentication** (`SO_PEERCRED`/`getpeereid`; the helper's only peer is the
...
- **Daemon-repair-unavailable parity (negative):** a Real-Nix capture records that Nix 2.34.8
  reports `repairPath is not supported by store 'daemon'` for both the broker uid and root.
  Fake `NixAdapter` exposes no repair method. The positive two-phase path runs only through
  the root helper's fixed `--store local` executor and is parity-checked by the privileged lane
...
- **Two-phase mutating repair parity (privileged VM proves Real Nix 2.34.8 behavior):** the
  ...
  --store local store repair`, one path at a time, `max-jobs=0`, `builders` empty, managed pinned
...
| Remove final selector | remove→stage→commit | coherent manifest/lock and generation `outputs` become empty; the empty forest is retained and atomically activated; the rooted landmark is journaled without sending the helper an invalid empty root-set request; restart recovery is idempotent |
...
| AC-S21 | **Daemon repair is unavailable (negative parity):** `NixAdapter` has no repair method, and Real Nix 2.34.8 proves `nix store repair` against the daemon reports `repairPath is not supported by store 'daemon'` for both the unprivileged broker and root. Granting trust is therefore not a repair mechanism. The positive repair path must be the helper's fixed `--store local` executor (§4.1.2/AC-S28) | contract (negative) + privileged VM |
| AC-S22 | **Helper grammar cannot widen:** every public input to `MaintenanceAdapter` (`publish_root_set`/`remove_root_set`/`repair_store_paths`) is rejected unless it round-trips through the validated `RootSet` / generation-id / typed **nonempty, sorted, de-duplicated** `StorePath` set / helper-issued maintenance capability. The private path-free generation-transition frame additionally carries only authenticated uid, distinct source/destination generation ids, and bounded unique retained `RootName` values; the helper reloads the durable source and refuses any unknown name, so the frame cannot add or rewrite a target. Raw path/installable/drv/expression/flake/argv/option/substituter/key/verb bytes are refused at construction and never reach argv. The Phase-A cache-only argv (`max-jobs=0`, `builders` empty) and Phase-B approved argv (bounded nonzero `max-jobs`, `builders` empty) are each byte-constant across the fuzz corpus; a closed-grammar property/fuzz lane asserts the surface cannot widen Fake-side or Real-side (§4.1.2/§4.3) | unit (property) + fuzz + privileged VM |
...
| AC-S24 | **Platform socket/config/store contracts (plan 07 §5.2/§7.4):** the rendered daemon `nix.conf` carries `trusted-users = root` and `allowed-users = pkg-nix-broker` (the broker is the sole `allowed-user`, root the sole `trusted-user`). On Linux the private daemon socket (systemd `Socket`) is `root:pkg-nix-broker` mode `0660` with its parent dir `0750`; on macOS the (self-created) socket may be `0666` but its **parent** dir is `root:pkg-nix-broker` `0750`, so traversal to the socket is impossible for an ordinary user (the `allowed-users` gate backs it). The macOS public broker path has search-only ordinary traversal (`service-root 0711` → `run 0751` → `broker 0771`) and a `0666` socket while directory writes remain broker-only; the helper path is `0750`/`0660`, and the root helper launchd job uses primary group `pkg-nix-broker`. An ordinary user can reach only the broker, and **cannot** connect to the raw daemon/helper or exec bundled Nix/helper. Clean modern macOS install additionally proves the encrypted APFS `/nix` volume, exact synthetic entry, root-only keychain item, boot mount, receipt ownership, and exact rollback. The product installs **no** automatic GC scheduler (no systemd timer / launchd job / `nix.conf` auto-GC key) and, on uninstall, removes only its own scheduler artifacts | unit (renderer/plist/manifest) + integration (privileged VM) + platform |
...
| AC-S28 | **Two-phase mutating store repair correctness (plan 05 §10; Nix 2.34.8):** the authoritative Real lane first proves daemon repair is unsupported, then exercises the helper's fixed `nix --store local store repair` path. After broker read-only verification detects damage: (a) Phase A repairs one path with `max-jobs=0`, empty `builders`, and pinned trust; a signed cache hit restores, while an intentionally uncached derivation returns typed `CacheMiss` and remains corrupt without building; (b) Phase B requires a separately approved plan digest, bounded nonzero jobs, empty `builders`, broker build mutex, and shared GC permit, then restores the uncached path; (c) fixed post-repair content verification and final broker recursive verification govern success; (d) repair never creates/swaps a generation; (e) only sanitized typed outcomes leave the service boundary. The helper never accepts a store selector | e2e (Fake) + privileged VM (Real, authoritative) + parity |
...
`connect()` to the raw daemon socket and **cannot** exec the bundled `nix`/helper. On macOS the
public broker endpoint remains reachable through search-only `0711`/`0751`/`0771` ancestors
and a `0666` socket, while the helper stays private (`0750` parent, `0660` socket,
root process with primary group `pkg-nix-broker`). The lane also proves clean-host encrypted
APFS `/nix` provisioning, exact `synthetic.conf` merge, root-only keychain ownership,
boot-time mount, receipt-last state, and exact rollback. It also asserts
...
- `[NIX-MANUAL]` — `nix build --json`; **root-level** `nix path-info --json --json-format 2`
  ...
  (used to build fixtures and the Real lane); Nix 2.34.8's daemon protocol reports
  **`repairPath` unsupported even for root**, so the two-phase repair pins `--store local`
  inside the root helper (`Store::repairPath` substitutes first,
