...
- request **`--json`** and parse structured output (never human output),
...
>
> ⚠️ *Two compatibility caveats, both isolated behind the single versioned `nix-driver` adapter (ARCH-INV-01) and pinned to the managed Nix runtime version:* (a) **`--log-format internal-json`** is Nix's documented machine-readable log channel (doc 04 §5.3/§10.1) but is *nominally internal* — `pkg` parses it only inside the adapter and re-pins it with each managed-Nix upgrade (validated by the parity job). (b) **`nix store verify`** flag specifics (NAR-integrity-only vs. trust-required mode, and whether a JSON mode exists) are **pinned for the chosen managed Nix runtime and validated by the Fake↔Real parity job** (doc 09 §4.3; this is the SPK-02 continuous-enforcement mechanism of doc 00 §11 — not a standalone spike); the table shows the attested `--recursive` / `--repair` form only, and avoids unverified flags such as a standalone `--all`, `--no-trust`, or `--json`.
