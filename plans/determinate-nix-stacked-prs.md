...
- **Status:** alpha.52 instructions were published. Superseded by PUBLIC-19.
...
- **Status:** merged in [PR #54](https://github.com/spa5k/pkg/pull/54).
  Published in alpha.53. The original alpha.52 failure log does not establish the transport cause.
...
- **Status:** release preparation merged in [PR #56](https://github.com/spa5k/pkg/pull/56).
  Published as [alpha.53](https://github.com/spa5k/pkg/releases/tag/v0.1.0-alpha.53).
...
### PUBLIC-19: alpha.53 installation instructions

- **Status:** alpha.53 links, verified installer digests, and package/system removal instructions are current.
- **Purpose:** give users the verified alpha.53 installer and current package/system removal commands.
- **Owns:** published version links, the rendered installer with verified digests, command examples, release notes, and plan status.
- **Depends:** PUBLIC-18.
- **Tests & gates:** asset signatures, installer byte comparison and verification mode, shell syntax, renderer and documentation tests, docs links, and hosted checks; A primary/security and E cross-area review.
...
- **DN-16** blocks macOS deletion until Apple Silicon proves Base Nix install, terminal vendor uninstall, package operations, interruption and crash behavior, real reboot behavior, and the documented failure cases. Run `33256299782` stopped at the hosted input gate because the channel and proof-input release manifests were not byte-identical. No self-hosted runner or product mutation started.
  Run `33264502945` passed every hosted gate and then failed in slot 1 prepare during the native Alpha.12 install. The product gave the vendor installer a `0700` root-only `TMPDIR`, and the vendor's unprivileged Nix build users could not stat it. The signed fix `76d6a455` moves the macOS vendor `TMPDIR` to the traversable, root-owned `/private/var/db/pkg-install-tmp` with no group or other write bits. Workflow 14 pins the corrected Alpha.14/15 pair from that commit. Workflow-14 run `33268434479` passed every hosted gate and the vendor installer then completed all nine receipt actions on the real VM, but the install still failed one second after the handoff was accepted: the fresh-install `nix-root` classification rejected the vendor-created `/nix` because neither `store_created` nor `existing_managed_install` was set for a handoff accepted during the run. A private diagnostic on a real VM then proved two more macOS-only defects and their fixes at signed commit `dd66653`: handoff acceptance now raises `existing_managed_install`, and the Determinate adapter materializes the pinned nixpkgs source with `--option lazy-trees false` because Determinate Nix omits the source store path from lazy-tree flake metadata. The same diagnostic proved the complete flow end to end: fresh install, healthy broker, `pkg install hello`, `pkg install ripgrep`, and `pkg remove`. Workflow 15 pins the corrected Alpha.16/17 pair from that commit. Workflow-15 run `33276655652` then passed every hosted gate and completed the full native Alpha.16 install on the real VM (`fresh-release-n-install` passed, 76.95 seconds), but the harness failed its immediate post-install launchd check: the broker's first KeepAlive relaunch was still in flight. The product design relies on launchd KeepAlive for service restart, so the harness now waits the bounded KeepAlive window instead of racing it. The pair, product commit, and releases are unchanged. Workflow 16 re-dispatches the same pair with that harness fix. Workflow-16 run `33277673891` completed the install, service wait, ownership receipt, and exact-job checks, but the exact repeat-install no-op failed: the clean-host preflight binds the installed manifest's prior asset states twice, and the second bind treated the identical duplicate as a conflict. The signed fix `118f41d` on `fix/dn16-macos-idempotent-prior-state` makes rebinding identical prior states idempotent while still refusing conflicting states. Workflow 17 pins the corrected Alpha.18/19 pair from that commit. Workflow-17 run `33280600444` then reached the repeat no-op again and exposed two deeper macOS-only defects: the receipt phase rebuilt a run-local manifest, so the exact receipt from the original installation was treated as drift and rewritten, and the accepted-handoff `Existing` path dropped its authenticated bundle and forced a second channel commit that cannot succeed against already-committed state. The signed fix `4de8b12` on `fix/dn16-macos-idempotent-prior-state` records the receipt as the canonical installation manifest, retains the bundle on the accepted-handoff path, and matches the Linux `Existing` receipt arm exactly. A private diagnostic on the real VM proved the repeat install now completes as an exact no-op. Workflow 18 pins the corrected Alpha.20/21 pair from that commit. Workflow-18 run `33299320534` then passed the repeat no-op and exposed the eighth macOS-only defect: the offline Product Asset Repair passed the receipt's recorded digest as the required current-bytes digest, so repairing a damaged file always failed. Linux `ReplacementAuthority::RepairExisting` passes no prior digest for an explicit repair; the signed fix `cbd3494` does the same for the macOS backend. A private diagnostic on the real VM proved the repair completes and restores the damaged product CLI byte-exactly. Workflow 19 pins the corrected Alpha.22/23 pair from that commit. Workflow-19 run `33303051799` then passed the fresh install, repeat no-op, offline Product Asset Repair, and representative package state (`hello` and `ripgrep` installed through the production CLI), and the staged N-to-N+1 upgrade step stopped on a harness defect: the proof script called `/bin/chown`, but macOS keeps `chown` in `/usr/sbin`. The harness now uses the exact `/usr/sbin/chown` path. The sealed pair, product commit, and releases are unchanged. Workflow-20 run `33304118626` then completed the full slot 1 prepare phase for the first time (fresh install, repeat no-op, offline repair, representative package state, offline N-to-N+1 upgrade with byte-compared snapshots, and the protected continuation record) and passed the resume checks through continuation-reboot, resumed-offline state, preserved state, and service activation. The first package transaction then hit the broker's launchd KeepAlive restart window and was refused with ENGINE_UNAVAILABLE. The harness now waits for the managed engine to answer before the first package transaction. Workflow 21 re-dispatches the same pair with that readiness wait. Workflow-21 run `33306000620` repeated the same refusal: the readiness probe used `pkg list`, which does not exercise the broker write path. The probe now uses the idempotent `pkg update`, which requires the managed engine. Workflow-22 run `33322138086` then stopped at its hosted input gate because the ephemeral Quick Tunnel hostname died after a long host gap. The replacement tunnel has a new URL, so every binary now embeds `https://stakeholders-friday-exhibits-pubmed.trycloudflare.com`. Workflow 23 pins the rebuilt Alpha.24/25 pair from the same signed product commit `cbd3494`. Workflow-23 run `33324617235` passed the full slot 1 prepare and every resume check through service activation; the first resume lifecycle transaction then correctly refused to reinstall the representative packages that the prepare phase had installed by design. The harness now starts the resume lifecycle from the authenticated channel refresh, matching the documented resume contract of update, upgrade, rollback, repair, and garbage collection. Workflow 24 re-dispatches the same pair with that correction. Workflow-24 run `33326812145` then passed the full slot 1 prepare, every resume check, the complete package lifecycle, the native generation remove, the structured uninstall refusal, and the plain terminal uninstall. The final-absence check found only the empty boot-time synthetic `/nix` node: the vendor uninstall had already revoked its `synthetic.conf` creation, unmounted and deleted the Nix Store volume, and emptied the node, but the sealed macOS root volume cannot remove a synthetic node at runtime. The harness now verifies exactly what macOS permits before a reboot: `/nix` is not mounted, holds no entry, and `synthetic.conf` no longer creates it, while every runtime-removable path must be fully absent. Workflow 25 re-dispatches the same pair with that check. The sealed pair, product commit, and releases are unchanged. The sealed pair, product commit, and releases are unchanged.
