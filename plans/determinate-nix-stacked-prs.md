...
- **DN-16** blocks macOS deletion until Apple Silicon proves Base Nix install, terminal vendor uninstall, package operations, interruption and crash behavior, real reboot behavior, and the documented failure cases. Workflow-13 run `33264502945` passed every hosted gate and then failed in slot 1 prepare during the native Alpha.12 install: the product gave the Determinate installer a `0700` root-only `TMPDIR`, and the vendor's unprivileged Nix build users could not stat it while they set up a build environment. A private diagnostic reproduced the failure and proved that a root-owned traversable vendor `TMPDIR` with no group or other write bits installs successfully. The product fix moves the macOS vendor `TMPDIR` to `/private/var/db/pkg-install-tmp` (root-owned, mode `0755`, no group or other write) and keeps every other private directory at exact mode `0700`. Linux paths and modes stay unchanged. Workflow-14 run `33268434479` then passed every hosted gate, and the vendor installer completed all nine receipt actions on the real VM, but the install still failed one second after the handoff was accepted: the fresh-install `nix-root` classification rejected the vendor-created `/nix` because neither `store_created` nor `existing_managed_install` was set for a handoff accepted during the run. The signed fix branch `fix/dn16-macos-accepted-handoff-nix-root` sets `existing_managed_install` when the handoff is accepted, which is exactly the state that flag describes. A private diagnostic on a real VM then proved that the fresh install completes with that fix, that the broker serves packages, and that `pkg install hello`, `pkg install ripgrep`, and `pkg remove` work end to end. The same diagnostic found one more macOS-only defect: Determinate Nix defaults to lazy trees, which omit the materialized source store path from `nix flake metadata --json`, so every package acquisition failed at the fetch stage. The fix adds `--option lazy-trees false` to the Determinate adapter's pinned-source metadata command only; the Linux-managed adapter keeps its proven command.
