...
- **ARCH-INV-01** Normal Nix interaction is carried exclusively by the product-owned private broker across three layers: closed user CLI → broker framed RPC; controlled broker → bundled `nix` subprocess; and bundled CLI → daemon native protocol. Users never open the daemon socket or launch the bundled Nix binary. Repair is the single narrow execution exception: pinned Nix 2.34.8's daemon protocol rejects `repairPath` even for root, so the broker verifies read-only through the daemon and the root helper alone executes **fixed `nix --store local store repair` commands**. No store selector, public/raw path, installable, expression, argv, option, substituter/key, environment override, output selection, or arbitrary verb crosses the framed boundary. The helper resolves an opaque expiring single-use capability into a `VerifiedRepairScope` bound to uid, an existing rooted generation, exact typed paths, plan digest, policy version, and mode. Phase A fixes `max-jobs=0` and empty `builders`; Phase B requires ordinary preview/approval for every rebuildable output, bounded nonzero jobs, empty `builders`, the broker build mutex, and a shared GC-inhibit permit. Exactly one sanitized typed outcome per trusted path returns; raw logs stay service-private and human CLI output is never parsed.
...
flowchart TB
  ...
  NixDrv -. "repair ONLY if read-only verify confirms corruption:<br/>closed opaque request → fixed local-store repair as root<br/>(--store local; Phase A cache-only max-jobs=0 / builders empty;<br/>Phase B approved build over all outputs)" .-> Helper
...
flowchart LR
  ...
  Broker -. "repair ONLY if read-only verify confirms corruption:<br/>closed opaque request → fixed local-store repair as root<br/>(--store local; Phase A cache-only max-jobs=0 / builders empty;<br/>Phase B approved build over all outputs;<br/>daemon repair unsupported)" .-> Root
  ...
  Root -. "nix --store local store repair<br/>(daemon repairPath unsupported;<br/>fixed store selector)" .-> Store
...
>
> ⚠️ *Three compatibility caveats, all isolated behind the single versioned `nix-driver` adapter (ARCH-INV-01) and pinned to the managed Nix runtime version:* (a) **`--log-format internal-json`** is Nix's documented machine-readable log channel (doc 04 §5.3/§10.1) but is *nominally internal* — `pkg` parses it only inside the adapter and re-pins it with each managed-Nix upgrade (validated by the parity job). (b) **`nix store verify`** mode specifics (NAR-integrity-only vs. trust-required) and whether **`verify`** or **`gc`** expose a JSON mode at all are runtime-dependent; the adapter **does not assume a JSON payload** for either. **Repair command & privilege split (verified Nix 2.34.8):** the modern mutating command is **`nix store repair`** (there is **no** modern `nix store verify --repair` combination — `verify` is read-only, `repair` mutates); `Store::repairPath` first tries Repair-mode substitution and, failing that with a valid deriver, rebuilds **all** outputs (`bmRepair`). The daemon protocol reports `repairPath is not supported by store 'daemon'` even for root. Therefore the unprivileged broker may run `nix store verify` through the daemon but never issues repair; the root helper executes the two phases with an explicit fixed **`--store local`**, accepts no store selector, and is the sole local-store repair executor (Phase A per-path cache-only with managed pinned substituters/keys, `max-jobs = 0`, `builders` empty; Phase B ordinary build preview/approval with a `RepairBuildPlan`/digest over every rebuildable output, holding the broker build mutex + GC-inhibit permit, run locally with bounded nonzero `max-jobs`, `builders` empty; §12.4). (c) For any command with no machine payload — `verify`, `repair`, `gc`, and any other status-less op — `pkg` checks **exit status plus independently validated filesystem/store postconditions** (e.g. a corrupt path verifies clean after repair; reachable closures survive `gc`; the gcroot symlink exists and resolves to a live store path), **never by parsing human stdout**, and **never pretends such a command emits JSON**. Each form is pinned for the chosen managed Nix runtime and validated by the Fake↔Real parity job (doc 09 §4.3; this is the SPK-02 continuous-enforcement mechanism of doc 00 §11 — not a standalone spike).
...
- **repair** (doc 05 §10; two-phase privilege-split per ARCH-INV-01/§11). `nix store verify` and `nix store repair` are **separate** modern commands: `verify` is read-only; `repair` mutates. `Store::repairPath` first tries Repair-mode substitution and, if unavailable and a valid deriver exists, rebuilds **all outputs** of that deriver (`bmRepair`). In pinned Nix 2.34.8, the daemon protocol rejects `repairPath` even when the client is root. The broker therefore performs only read-only verification through the daemon; every repair mutation is a fixed **`nix --store local store repair`** operation in the root helper.

  1. The broker verifies recursively and computes the damage set from broker-held generation state.
  2. The helper resolves an opaque, expiring, single-use capability bound to caller uid, an existing rooted generation, the exact validated closure paths, `RepairBuildPlan` digest, `policyVersion`, and mode. It accepts no public/raw path, **store selector**, installable, expression, argv, option, substituter/key, environment override, output selection, or arbitrary verb.
  3. **Phase A:** repair one path at a time against the fixed local store with managed pinned substituters/keys, `max-jobs=0`, and empty `builders`. A clean post-repair content verification means restored; an otherwise responsive store means cache miss and stops before build.
  4. **Phase B:** after ordinary build preview and explicit approval covering every output Nix may rebuild, repair locally with bounded nonzero `max-jobs`, empty `builders`, the broker machine-wide build mutex, and a shared GC-inhibit permit.
  5. A fresh broker-mediated recursive verification governs final success. Only sanitized per-path outcomes cross the service boundary; raw Nix logs remain service-private. Repair never touches user data outside `/nix` and GC remains in the unprivileged broker.
...
- **Privilege minimization (ARCH-INV-05/ARCH-INV-07):** privilege is split across the unprivileged private broker (sole general daemon client and GC-root mediator) and the root helper (sole root-set filesystem writer and sole fixed local-store repair executor). The helper is not setuid, accepts only closed broker requests, pins `--store local`, and exposes no caller-controlled store or command material. The CLI holds no daemon/socket privilege. Detailed framed RPC and capability surfaces live in docs 07/08/13.
