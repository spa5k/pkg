...
| CHANNEL | Signed channel descriptor client (mature update metadata). **Refresh of machine-global channel is broker-mediated**; user-side does verified reads only | `02` | invoking user (verified reads); broker-mediated refresh | per-user cache (reads); `/var/lib/pkg/broker-home/channel` (machine-global, broker-owned) |
| INDEX | Disposable search/list/info index (derived from pinned Nixpkgs). **Refresh is broker-mediated**; verified reads may be user-side | `03` | invoking user (verified reads); broker-mediated refresh | per-user cache (reads); `/var/lib/pkg/broker-home/index` (machine-global, broker-owned) |
...
| HELPER | Privileged installer/root helper (**sudo/polkit/AuthorizationServices or narrow root service; NOT a Linux setuid binary in v1**); **sole root-set filesystem writer** for pkg GC roots and **sole fixed local-store repair executor**. Nix 2.34.8's daemon protocol rejects `repairPath` even for root, so repair always pins `--store local` inside the helper and no store selector crosses the framed boundary. It accepts only a broker-issued opaque, expiring, single-use capability bound to uid, existing rooted generation, exact validated closure paths, plan digest, policy version, and mode. Phase A is per-path cache-only (`max-jobs=0`, empty `builders`); Phase B requires the ordinary build preview/approval and runs locally with bounded nonzero jobs and empty `builders`. It accepts no public/raw path, store selector, installable, expression, argv, option, substituter/key, environment override, output selection, or arbitrary verb; only sanitized per-path outcomes leave the service boundary. The user CLI never calls it directly | `07` | root | — |
...
flowchart TB
    ...
    BROKER -. "repair ONLY if read-only verify confirms corruption:<br/>closed opaque capability → fixed local-store repair as root<br/>(Phase A cache-only max-jobs=0 / builders empty;<br/>Phase B approved build over all outputs)" .-> HELPER
    HELPER -. "nix --store local store repair<br/>(daemon repairPath unsupported;<br/>store selector is fixed)" .-> STORE
    CORE --> STATE
    CORE --> CUR
```
...
3. **User space → product service (BROKER) → privileged (NIXD/HELPER).** The CLI crosses
   ...
   **service-only** and never user-connectable (D-18/INV-11), and only the unprivileged
   ...
   JSON); the one operation unavailable through that daemon protocol — **repair** (Nix 2.34.8 reports `repairPath is not supported by store 'daemon'`, including for root) — is delegated to HELPER as a **two-phase fixed local-store** operation (T-INST-7). The broker authenticates the caller uid; HELPER is a narrow root boundary for
   install/service-control, is the **sole filesystem writer** of GC-root sets, and is the **sole fixed local-store repair executor** running `nix --store local store repair` as root, accepting a
...
> Binary Caches".
>
> **ℹ️ FACT (modern new-CLI model pkg uses).** The modern commands are **`nix store verify`** (read-only NAR-integrity check) and the **separate** mutating command **`nix store repair`**. `Store::repairPath` re-fetches/rebuilds from trusted sources, but Nix 2.34.8's daemon protocol reports that `repairPath` is unsupported even when invoked by root. `pkg` therefore verifies through the daemon and repairs only through the helper's fixed `--store local` invocation. (The legacy `nix-store --verify --repair` form is not used.) `[NIX-MANUAL]`
>
...
> More specifically for repair, pinned Nix 2.34.8's daemon store does not implement
> `repairPath` at all: both an allowed user and root receive `operation 'repairPath' is not
> supported by store 'daemon'`. **Consequence:** the product broker can verify through the
> daemon but cannot repair, and granting it trust would not solve this. Repair is delegated
> to the root helper as a two-phase operation pinned to **`--store local`** (§6.1 T-INST-7,
> `01` §12.4). The helper accepts no caller-controlled store selector.
>
...
> root-equivalent and Nix 2.34.8's daemon protocol **does not implement `repairPath`, even for
> root**, the broker — kept deliberately unprivileged — cannot repair through the daemon;
...
> helper, which is the **sole fixed local-store repair executor** and runs
> **`nix --store local store repair` as root in two phases**. **Phase A (cache-only):** per-path repair against a **nonempty
...
| **T-INST-7** | Confused-deputy local-store repair: an attacker tries to turn the root helper into an arbitrary local-store/argv primitive | EoP/Tampering | Local unprivileged or broker-adjacent input | Reach helper / craft repair request | H | The daemon protocol cannot perform repair, so the helper must access the managed store locally as root | The helper is the **sole fixed local-store repair executor**. It always invokes `nix --store local store repair`, never accepts a store selector, and operates one path at a time from a nonempty sorted `VerifiedRepairScope` derived from broker-held rooted generation state. The opaque capability is expiring and single-use, and binds uid, generation, exact typed paths, plan digest, policy version, and mode; stale/replayed/mismatched/cross-uid requests fail closed. Phase A fixes `max-jobs=0` and empty `builders`; Phase B requires fresh preview/approval for every rebuildable output, bounded nonzero jobs, empty `builders`, the broker build mutex, and a shared GC-inhibit permit. The helper accepts no public/raw path, installable, expression, argv, option, substituter/key, environment override, output selection, arbitrary verb, or store URL. A fixed post-repair content verification classifies each outcome; only sanitized typed outcomes cross the boundary and raw logs remain service-private | L | `01`,`05`,`07`,`13` |
...
|----|--------|--------|--------|----------|----------|------|
| **T-DAEMON-1** | Untrusted client drives the managed `nix-daemon` to build/eval arbitrary expressions or pull from arbitrary substituters | EoP/Tampering | `trusted-users` too broad; user supplies `--expr`/`--substituters` | The raw daemon socket is **service-only** and reachable only by the unprivileged broker (an `allowed-user`, never a `trusted-user`); end users cannot connect. The broker forwards only closed requests and uses `restrict-eval` plus pinned `allowed-uris`. Repair is not an alternate daemon privilege path: Nix 2.34.8's daemon protocol does not implement `repairPath`; it is confined to the helper's fixed `--store local` operation under T-INST-7 | L | `01`,`04` |
...
|---------------|--------------------|-------------------------|---------------|-----------|
| Installer/helper (T-INST) | — | closed validated broker requests only; no setuid in v1; sole root-set writer and sole fixed `--store local` repair executor. Capabilities bind uid, rooted generation, exact typed paths, plan digest, policy version, and mode; no store selector or raw command material; cache-only and approved-build phases use fixed bounded options; sanitized typed outcomes only (T-INST-7/T-INST-8) | `07`,`13` | `09` security lane |
| Daemon/IPC (T-DAEMON) | socket perms, `trusted-users=root` only | **unprivileged broker is the sole general daemon `allowed-user`**; bundled CLI→daemon is native protocol; strict serde, size caps, no expr/substituter passthrough. Repair is not supported by the daemon protocol and is confined to the helper's fixed `--store local` operation (T-INST-7) | `01`,`04` | `09` contract+fuzz |
...
- **AC-S31** (Repair privilege split & confused-deputy resistance; Nix 2.34.8). A real isolated test proves daemon repair fails with `repairPath is not supported by store 'daemon'` even for root. The broker verifies read-only through the daemon; only the helper can mutate repair state, using fixed `nix --store local store repair` commands. The helper accepts no store selector or raw command material, resolves only a server-side `VerifiedRepairScope`, consumes a uid/generation/paths/plan/policy/mode-bound single-use capability, and returns exactly one sanitized typed outcome per trusted path. Any cardinality mismatch, replay, drift, or cross-uid request fails closed.
- **AC-S32** (Two-phase repair phase boundary; Nix 2.34.8). The real isolated test corrupts an intentionally uncached derivation: Phase A uses `--store local`, `max-jobs=0`, and empty `builders`, returns `CacheMiss`, and leaves the corrupt content unbuilt; Phase B uses the separately approved plan digest, bounded nonzero jobs, and empty `builders`, then restores content and verifies it clean. Unit tests assert the exact fixed argv and prove a failed repair command or unresponsive local store is never downgraded to `CacheMiss`.
