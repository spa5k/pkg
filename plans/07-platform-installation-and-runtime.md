...
2. Create the **`pkg-nix-broker`** unprivileged service user/group. Before
   ...
   dynamic UUID plus fixed keychain selector in the separate root:wheel `0600`
       ...
       attempts to authenticate host-generated bytes. The production writer accepts
...
21. The privileged runtime provisioner now has a production bootstrap-daemon
    ...
    After the platform service passes readiness, transaction finalization stops
    and reaps this bootstrap daemon before it releases runtime rollback ownership.
    A stop failure remains fail-closed and triggers exact runtime rollback.
