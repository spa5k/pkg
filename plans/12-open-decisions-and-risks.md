...
- **Status:** Proposed (pending **S3 / PR-7**). S5 supplies native sandbox/build evidence and a 2026-08-10 Complete real S3 Detect confirms Nix 2.34.8, full Xcode, all Apple tools, and 32 `_nixbld*` users; however broker-run cache Preflight remains Pending and the host has zero Developer ID Application/Installer identities, so real signing/notarization evidence is still Pending. Supersedes the earlier "macOS is binary-only in v1" framing.
...
- **Consequences:** Activation is store-independent, deterministic, and needs no per-generation
  ...
  **no remaining E-track `buildEnv` reconciliation**. The managed **private broker** boundary is
  ...
  sole fixed local-store repair executor** running two-phase `nix --store local store repair` as root; the broker is
  an allowed-user but **not** a trusted-user (root is the sole trusted-user). The full privilege
  split + two-phase repair + capability + admission/log rules are fixed in D-19/DR-017. The
  boundary and its framed-RPC contract are **accepted**: PR-39 closed the wire-design milestone
  with the normative contract in doc 13. Real OS transport bindings (PR-27/28), Real-Nix
  execution (PR-36), and two-phase repair (PR-30) remain downstream. Security residuals (T-PATH-*,
...
### DR-017 — Broker/helper privilege split & two-phase store repair (accepted; wire contract closed by PR-39)
- **Status:** **Accepted.** The full privileged two-phase mutating store repair is an unconditional V1 milestone (Phase 0 read-only verify via broker → Phase A cache-only repair via helper → Phase B approved local rebuild via helper; `00` D-19/INV-12), so PR-30 remains in scope. PR-39 completed the exact framed RPC / peer-auth / operation-lifecycle / child-containment / capability-token / restart-handshake contract; doc 13 is normative. PR-39 continues to gate broker/helper integration (PR-27/28), two-phase repair (PR-30), and Real-Nix execution (PR-36). The verified non-atomic residual remains **RISK-22**.
...
- **Decision (boundary facts, accepted):** (1) The **unprivileged singleton broker** is the sole general Nix daemon client and bundled-CLI spawner for normal operations. (2) The **privileged root helper** is the sole root-set filesystem writer and **sole fixed local-store repair executor**. Pinned Nix 2.34.8 reports `repairPath is not supported by store 'daemon'` even for root, so every repair mutation pins `--store local` inside the helper and no store selector crosses the wire. (3) **Two-phase repair:** broker read-only verify → helper cache-only repair (`max-jobs=0`, empty `builders`) → separately approved local rebuild (bounded jobs, empty `builders`, broker build mutex and shared GC permit). (4) The capability is opaque, expiring, single-use, and bound to uid, rooted generation/closure, typed paths, plan digest, policy version, and mode; mismatches fail closed. (5) Build/GC admission are broker-internal gates; the per-user state-mutation lease remains a filesystem `flock`. (6) Raw Nix logs are service-private; public/user-state logs are sanitized NDJSON.
- **Consequences:** The boundary is fixed and citable (D-19, INV-12), and doc 13 fixes the closed wire grammar downstream transports must implement without widening. Repair is **verified non-atomic** — cache repair deletes the live path before restore and an approved local repair moves the old output aside before replacement — disclosed as residual **RISK-22**, mitigated by user initiation, an availability warning, per-path journal, final read-only verify, cache-only auto-resume, and fresh approval for local rebuilding. Confused-deputy repair is **T-INST-7** (mapped to RISK-10).
- **Owner:** E + A. **Source:** `[NIX-MANUAL]` plus isolated Nix 2.34.8 execution (store verify and repair are separate; daemon `repairPath` is unsupported even for root; fixed `--store local` repair succeeds); `01` ARCH-INV-01/05; `05` §10; `08` T-INST-7.
...
| **S3** (PR-7) | Do v1 attrs substitute for `x86_64-darwin`/`aarch64-darwin`, **and** are native macOS local builds viable (sandbox, `_nixbld` users, Xcode toolchain, the honest resource boundary with **no** per-build cap in stock Nix, fail-closed)? Is a notarized installer feasible? | Availability matrix for fixture attrs; a real native sandboxed Darwin build under `_nixbld`; notarized installer/runtime builds & validates. | PR-26, PR-28, PR-36 (macOS lane) | end of M0.5 | Proposed (native build + Complete Detect observed; broker cache Preflight and Developer-ID notarization Pending) | DR-003 |
