...
if [ -n "$artifact_output" ]; then
    candidate="$artifact_output/pkg-v0.1.0-alpha.51-linux-x86_64.tar.gz"
...
cp "$repo/tests/linux-clean-host/pkg-proof-server.py" \
    ...
    "$artifact_context/"
cp -a "$artifact_context/v0.1.0-alpha.51" "$artifact_context/publication-1/"
cp -a "$artifact_context/v0.1.0-alpha.51" "$artifact_context/publication-2/"
cp "$raw_stage/binaries/pkg-install-n-plus-1" \
    "$artifact_context/publication-2/v0.1.0-alpha.51/pkg-installer-x86_64-linux"
...
root = pathlib.Path(sys.argv[1])
path = root / "v0.1.0-alpha.51/pkg-installer-x86_64-linux"
manifest = json.loads((root / "release-manifest.json").read_text())
record = next(
...
shipping_installer=/srv/pkg-release/v0.1.0-alpha.51/pkg-installer-x86_64-linux
...
'\''
    ...
    test "$(/usr/local/bin/pkg --version)" = "pkg 0.1.0-alpha.51"
    test ! -e /opt/pkg/nix
    test ! -L /opt/pkg/nix
...
test "$(docker exec "$container" /usr/local/bin/pkg --version)" = "pkg 0.1.0-alpha.51"
