...
if [ -n "$artifact_output" ]; then
    candidate="$artifact_output/pkg-v0.1.0-alpha.55-linux-x86_64.tar.gz"
...
cp "$repo/tests/linux-clean-host/pkg-proof-server.py" \
    ...
    "$artifact_context/"
cp -a "$artifact_context/v0.1.0-alpha.55" "$artifact_context/publication-1/"
cp -a "$artifact_context/v0.1.0-alpha.55" "$artifact_context/publication-2/"
cp "$raw_stage/binaries/pkg-install-n-plus-1" \
    "$artifact_context/publication-2/v0.1.0-alpha.55/pkg-installer-x86_64-linux"
...
product-upgrade	platform::linux::assets::tests::ordinary_upgrade_requires_different_release_and_prior_content_identity
...
product-asset-repair	platform::linux::assets::tests::repair_requires_same_release_and_created_product_ownership
product-asset-repair	platform::linux::assets::tests::repair_requires_a_receipt_and_non_files_never_gain_implicit_ownership
...
root = pathlib.Path(sys.argv[1])
path = root / "v0.1.0-alpha.55/pkg-installer-x86_64-linux"
manifest = json.loads((root / "release-manifest.json").read_text())
record = next(
...
shipping_installer=/srv/pkg-release/v0.1.0-alpha.55/pkg-installer-x86_64-linux
...
'\''
    ...
    test "$(/usr/local/bin/pkg --version)" = "pkg 0.1.0-alpha.55"
    test ! -e /opt/pkg/nix
    test ! -L /opt/pkg/nix
...
test "$(docker exec "$container" /usr/local/bin/pkg --version)" = "pkg 0.1.0-alpha.55"
