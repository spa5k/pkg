...
if [ -n "$artifact_output" ]; then
    ...
    candidate="$artifact_output/pkg-v0.1.0-alpha.2-x86_64-linux.tar.gz"
...
cp "$repo/tests/linux-clean-host/pkg-proof-server.py" \
    ...
    "$artifact_context/"
cp -a "$artifact_context/v0.1.0-alpha.2" "$artifact_context/publication-1/"
cp -a "$artifact_context/v0.1.0-alpha.2" "$artifact_context/publication-2/"
...
shipping_installer=/srv/pkg-release/v0.1.0-alpha.2/pkg-installer-x86_64-linux
...
docker exec "$container" sh -eu -c '
    ...
    test "$(/usr/local/bin/pkg --version)" = "pkg 0.1.0-alpha.2"
