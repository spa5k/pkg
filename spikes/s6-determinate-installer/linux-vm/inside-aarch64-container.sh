...
set -- "$installer" --diagnostic-endpoint "$endpoint" install linux --determinate --no-confirm --no-modify-profile --init none --extra-conf 'sandbox = false'
if [ "$target" = x86_64-linux ]; then
    set -- "$@" 'filter-syscalls = false'
fi
write_argv "$evidence/install.argv" "$@"
set +e
DETSYS_IDS_TELEMETRY=disabled "$@" >"$evidence/install.output" 2>&1
install_status=$?
set -e
