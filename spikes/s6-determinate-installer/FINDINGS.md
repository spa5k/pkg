...
- An empty string fails URL parsing, is reparsed as `file://`, and `FileTransport::new("")` fails to open a file. That error makes `diagnostics()` fall back to the dev-null feedback client (`src/diagnostics.rs:108-120`, `detsys-ids-client-0.7.0/src/transport/{mod,file}.rs`). So empty string = no report sent. The disable contract is Observed in help text and README; the fall-through chain is Observed in code; the practical "nothing is sent" result is Inferred.
- Extra kill switches (Observed in `detsys-ids-client-0.7.0/src/lib.rs`): `DETSYS_IDS_TELEMETRY=disabled` turns reporting off and prints a note; `DETSYS_IDS_TRANSPORT` overrides the ambient endpoint. The official telemetry doc confirms `DETSYS_IDS_TELEMETRY=disabled` as the opt-out. Source: `https://docs.determinate.systems/guides/telemetry/`. Observed.
...
| R-11 | Telemetry is off in automation | Diagnostics are on by default; opt-out needs `--diagnostic-endpoint ""` or `DETSYS_IDS_TELEMETRY=disabled` | §7 |
