...
grep -F 'tart run --no-graphics --no-audio --no-clipboard --no-keyboard --no-pointer --net-host "$vm_name"' "$host" >/dev/null || die "exact host-only Tart run flags missing"
grep -F 'write_argv "$out/vm-run.argv" tart run --no-graphics --no-audio --no-clipboard --no-keyboard --no-pointer --net-host "$vm_name"' "$host" >/dev/null || die "exact host-only VM run argv is not recorded"
grep -F -- '--net-softnet' "$host" >/dev/null && die "Softnet is forbidden for this preflight"
