...
Rust. `pkg` is a Rust CLI for Linux and macOS, implemented to alpha (technical preview; breaking changes may occur before v1). It drives a bundled, pinned Nix runtime as a subprocess and never exposes a raw Nix interface.
...
- `plans/` holds the design decisions; the Rust alpha implements them. See `plans/00-overview-and-decisions.md` and `plans/README.md`.
