...
async fn build_preview_publication(
    ...
    if output.exists() || sequence != 10 {
        return Err("usage: linux_proof_publication --preview OUTPUT INPUT STATE_DIR 10".into());
    }
    ...
    let manifest = serde_json::json!({
        "schemaVersion":1, "releaseId":"v0.1.0-alpha.6", "channelSequence":sequence,
...
async fn main() -> Result<(), AnyError> {
    ...
    if first == "--preview" {
        ...
        if arguments.next().is_some() {
            return Err(
                "usage: linux_proof_publication --preview OUTPUT INPUT STATE_DIR 10".into(),
            );
        }
    ...
    let candidate = |attr_path: &str, pname: &str, description: &str| IndexCandidate {
    ...
    let index = compress(
        build_index(
            metadata,
            vec![
                candidate("hello", "hello", "Print a greeting"),
                candidate("ripgrep", "ripgrep", "Search text"),
                candidate("fd", "fd", "Find files"),
                candidate("bat", "bat", "View text files"),
                candidate("tree", "tree", "List directory trees"),
                candidate("wget", "wget", "Download files"),
                candidate("git", "git", "Manage source history"),
                candidate("tmux", "tmux", "Use terminal sessions"),
                candidate("zoxide", "zoxide", "Navigate directories"),
                candidate("fzf", "fzf", "Find items with fuzzy search"),
                IndexCandidate {
